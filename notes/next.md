...
- Phase 10 (`Pipeline and Targeted Performance Design`) is open after the P10-S9 ALU-result-gating attempt on Linux. `npc/rtl/core/Core.v` remains at the accepted P10-S8 structure: selected 3-stage elastic in-order pipeline (`F/X/C`) plus IFU direct refill, ALU/branch comparator sharing, and split redirect decision/target state. P10-S8 passed spec smoke, directed/DiffTest regression, RT-Thread, and STA to a clean checked `680 MHz` (`690 MHz` fails by about `15 ps`). P10-S9 tried gating `xc_alu_result` with `wb_sel == NPC_WB_ALU`; it passed functional validation but regressed timing (`680 MHz` slack from `+0.006 ns` to `-0.073 ns`, reported top Fmax about `648.190 MHz`), so the RTL was reverted and the restored spec smoke passed. See `notes/p10-design-review.md` section 12.
