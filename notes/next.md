...
- Phase 10 (`Pipeline and Targeted Performance Design`) is open after a user-reviewed static branch prediction attempt. The accepted baseline is still P10-S9: a 4-stage `F/D/X/W` pipeline in `npc/rtl/core/Core.v` from commit `809d40d`, with clean checked STA at `770 MHz`, area `22122.240000`, `1629` DFFs, and RT-Thread `1955726` cycles. A no-BTB static predictor was attempted because BTB area was rejected: backward branches and `jal` were predicted in D, with late W-stage redirects for mispredict/`jalr`/`mret`. It became functionally correct (spec smoke, directed DiffTest, all 35 `cpu-tests`, and RT-Thread passed), but PPA regressed badly: `770 MHz` STA slack `-0.403 ns`, Fmax about `587.971 MHz`, area `23782.640000`, `1694` DFFs, RT-Thread `1988529` cycles. The cause was that the target-generation path moved to D and still fed `f_pc`/`dx_pred_pc`; it was slower and larger than the original X→F path. The RTL was reverted to the accepted P10-S9 baseline and clean spec smoke passed (`NPC_SPEC_RESULT status=good reason=uart_eot cycles=63 limit=400`). See `notes/p10-further-optimization.md` section "Static prediction attempt".
